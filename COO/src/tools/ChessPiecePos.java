 ...
 *
 */
// Enumération qui définit les positions initiales des pièces de jeu d'échec (des deux couleurs)
// Convention : BLANC en bas du plateau, NOIR en haut du plateau
public enum ChessPiecePos {
	...
	; // Le ; dans une énumération est nécessaire lorsque l'on définit des champs et des méthodes après les éléments de l'énumération

	// Champs de l'énumération (va servir à stocker les informations sur les pièces de jeu d'échec)
	public String nom;
	public Couleur couleur;
	public  Coord[] coords = new Coord[8] ;   // 8 cases pour les pions, les autres pièces n'utiliseront que les premières cases du tableau

	// Constructeur de l'énumération (va être appelé pour chaque élément de l'énumération pour initialiser les champs lors de la création de l'énumération)
	...
	// Méthode main pour afficher les éléments de l'énumération
	public static void main(String[] args) {
		for (int i = 0; i < ChessPiecePos.values().length; i++) {
			System.out.print(ChessPiecePos.values()[i].name() + " \t"); //name() est une méthode de l'énumération qui retourne le nom de l'élément de l'énumération (ex: "TOURBLANC")
